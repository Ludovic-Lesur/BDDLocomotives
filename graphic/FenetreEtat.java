...
public class FenetreEtat extends JFrame implements ActionListener {
	...
	/**
	    ...
	    @param pEngin		Engin moteur dont on doit modifier l'état, de type 'Engin'.
	...
	*/
	...
	/**
	    ...
	    @param e	Engin moteur dont on doit modifier l'état, de type 'Engin'.
	    @return		Aucun.
	*/
	...
	class ItemState implements ItemListener {
		...
		public void itemStateChanged(ItemEvent e) {
			...
			if (e.getSource() == choixPreservation) {
				preservationCourante = Association.values()[choixPreservation.getSelectedIndex()+1] ; // +1 car "INC" n'est pas dans la liste
			}
		}
	}
	...
	*/
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == ok) {
			engin.getSerie().modifyXML(Integer.toString(engin.getNumero()), etatCourant) ;
			engin.getSerie().modifyXML(Integer.toString(engin.getNumero()), champRenumerotation.getText()) ;
			engin.getSerie().modifyXML(Integer.toString(engin.getNumero()), dateMESCourante, true) ;
			engin.getSerie().modifyXML(Integer.toString(engin.getNumero()), dateRADCourante, false) ;
			engin.getSerie().modifyXML(Integer.toString(engin.getNumero()), preservationCourante) ;
