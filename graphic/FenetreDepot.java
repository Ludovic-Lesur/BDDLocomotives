...
public class FenetreDepot extends JFrame implements ActionListener {
	 ...
	 * @param pEngin
	 *            Engin moteur dont on doit modifier le depot, de type 'Engin'.
	 * @param pRemarque
	...
	public FenetreDepot(Interface pI, Engin pEngin, boolean pRemarque) {
		...
		fenetre = new JFrame();
		fenetre.setTitle("Modifier depot");
	...
	class ItemState implements ItemListener {
		...
		public void itemStateChanged(ItemEvent e) {
			if (e.getSource() == choixDepot) {
				depotCourant = Depot.values()[choixDepot.getSelectedIndex() + 1]; // +1
																					// car
																					// "INC"
																					// n'est
																					// pas
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == ok) {
			engin.getSerie().modifyXML(engin.getNumero(), depotCourant);
