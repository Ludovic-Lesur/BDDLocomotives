...
public class FenetreSuppression extends JFrame implements ActionListener {
	...
	/**
	    ...
	    @param pEngin			Engin dont on doit supprimer un attribut (vue ou remarque), de type 'Engin'.
	    @param pVue				Objet 'Vue' à supprimer si pBoolRemarque = 'false'.
	    						'null' sinon.
	    @param pRemarque		Objet 'Remarque' à supprimer si pBoolRemarque = 'true'.
	    						'null' sinon.
	    @param pBoolRemarque	Booléen indiquant l'attribut à supprimer.
	    							'true' = suppression d'une remarque.
	    							'false' = suppression d'une vue.
	    @return					Aucun.
	*/
	...
	*/
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == oui) {
			if (remarque == false) {
				enginCourant.getSerie().deleteXML(Integer.toString(enginCourant.getNumero()), vueCourante) ;
			}
			else {
				enginCourant.getSerie().deleteXML(Integer.toString(enginCourant.getNumero()), remarqueCourante) ;
			}
